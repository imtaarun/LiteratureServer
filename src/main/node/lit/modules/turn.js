...
function turn(res, card, opponent, player, io) {
    var gameData;
    Player.findOne(
        (err, doc) => {
            gameData = doc;
            if (!checkOneCard(gameData, card, player)) {
                if (check(gameData, card, player)) {
                    if (checkOneCard(gameData, card, opponent)) {
                        ...
                        //turn shifting
                        console.log('opponent not having the requested card');
                        var data = shiftPlayerTurn(shiftOpponentTurn(gameData, opponent), player);
                        data.save((err, doc) => {
                            if (!err) {
                                console.log('Init-Success');
                                io.emit('event', { type: 'start-message', msg: nameFinder(data, opponent) + ' has denied the request of ' + card + ' by ' + nameFinder(data, player) });
                                res.send(doc);
                            }
                            else {
                                console.log('Init-Error')
                    ...
                    //turn shifting
                    console.log('you not having the requested card pairs');
                    var data = shiftPlayerTurn(shiftOpponentTurn(gameData, opponent), player);
                    data.save((err, doc) => {
                        if (!err) {
                            console.log('Init-Success');
                            io.emit('event', { type: 'start-message', msg: nameFinder(data, player) + ' has commited a wrong call ' });
                            res.send(doc);
                        }
                        else {
                            console.log('Init-Error')
